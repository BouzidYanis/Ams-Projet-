"""
Mecanisme d'amelioration continue style Rasa X pour spaCy.

Cycle:
1) On log les interactions utilisateur + prediction du modele.
2) Un humain valide/corrige l'intent.
3) Les exemples valides sont reutilises au prochain entrainement.
"""
...
from app.nlu_models.francais.training_data import INTENTS
...
ENTITY_KEYS = ("sports", "lieux", "temps", "nombres")
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                # Ignore les lignes corrompues pour garder le flux resilient.
                continue
    return rows
...
def log_interaction(
    ...
    - corrected_intent=None  -> file pending
    - corrected_intent=value -> file validated
    """
    item = {
    ...
    if corrected_intent is None:
        ...
        if corrected_intent not in INTENTS:
            raise ValueError(f"Intent invalide: {corrected_intent}")
...
def validate_pending_item(index: int, corrected_intent: str) -> bool:
    """
    Valide/corrige un item pending par index (0-based).
    Retourne False si index invalide.
    """
    if corrected_intent not in INTENTS:
        raise ValueError(f"Intent invalide: {corrected_intent}")
...
def validated_to_training_data() -> list[tuple[str, dict]]:
    """
    Convertit feedback_validated.jsonl en format spaCy textcat.
    """
...
def record_entity_correction(
    ...
    """
    Enregistre une correction d'entité pour l'entrainement NER et ajoute un hint.
    Retourne True si la correction a été enregistree.
    """
    ...
    augmented_entities = {
        k: list(v or []) if isinstance(v, list) else []
        for k, v in (entities or {}).items()
    }
    if key not in augmented_entities:
        augmented_entities[key] = []

    existing = {str(v).strip().lower() for v in augmented_entities[key] if str(v).strip()}
    if value.lower() not in existing:
        augmented_entities[key].append(value)

    add_entity_hint(key, value)
    log_interaction(
        text=text,
        predicted_intent=predicted_intent,
...
def add_blocked_entity(entity_key: str, value: str) -> bool:
    """
    Ajoute une valeur d'entite a ignorer globalement.
    Retourne True si ajoute, False si deja presente ou invalide.
    """
    key = (entity_key or "").strip()
    val = (value or "").strip()
...
def apply_entity_blocklist(entites: dict) -> dict:
    """
    Retire les entites presentes dans la blocklist persistante.
    """
    data = load_entity_blocklist()
...
def add_entity_hint(entity_key: str, value: str) -> bool:
    """
    Ajoute une valeur d'entite qui sera reinjectee si elle apparait dans la phrase.
    Retourne True si ajoute, False si deja presente ou invalide.
    """
    key = (entity_key or "").strip()
    val = (value or "").strip()
...
def apply_entity_hints(text: str, entites: dict) -> dict:
    """
    Ajoute les entites de la whitelist si leur valeur apparait dans le texte.
    """
    ...
    # Preserver des cles custom eventuelles
    for key, values in entites.items():
...
def validated_to_ner_training_data() -> list[tuple[str, dict]]:
    """
    Convertit feedback_validated.jsonl -> format NER spaCy.

    Note: utilise les entites sauvegardees dans chaque interaction.
    """
    rows = validated_examples()
    label_map = {
        "sports": "SPORT",
