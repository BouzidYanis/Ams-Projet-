...
INTENT_PATTERNS = {
    ...
    "demander_mes_reservations": [
        r"\b(mes.*réservations?|réservations?.*moi)\b",
        r"\b(voir|afficher|montrer|liste).*\b(réservations?|booking)\b",
        r"\b(quels.*réservations?|quel.*réservations?|lesquel.*réservations?)\b",
        r"\b(j'ai.*réservé|j'ai.*booking)\b",
        r"\b(check|vérifier|confirmer).*\b(réservations?|mes.*cours|mes.*activités)\b",
        r"\b(info.*réservations?|information.*réservations?|détails.*réservations?)\b",
        r"\b(quand.*ma.*réservations?|quand.*mon.*cours)\b",
        r"\b(résumé|récapitulatif).*\b(réservations?|booking)\b",
        r"\b(lister|afficher).*\b(mes|ma).*\b(réservations?|booking|cours|activités)\b",
